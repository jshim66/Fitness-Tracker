...
const deletePrevPlannedDB = "DELETE FROM ActivityTable WHERE userID = ? amount < 0 and date BETWEEN ? and ?";
const getMostRecentPrevPlannedDB = "SELECT rowIdNum, activity, MAX(date), amount FROM ActivityTable WHERE userID = ?  amount <= 0 and date BETWEEN ? and ?";
 ...
 */
async function get_most_recent_planned_activity_in_range(userid,min, max) {
  try {
 ...
 */
async function get_similar_activities_in_range(userid, activityType, min, max) {
  try {
 ...
 * @param {number} max - ms since 1970
 */
async function delete_past_activities_in_range(userid, min, max) {
  try {
